...
pub fn interpolate(wavetable: &[[f32; WAVETABLE_SIZE]; FREQUENCY_STEPS], frequency: f32, phase: f32) -> f32 {
    ...
    let wavetable_index = (frequency.ln() / 2_f32.ln()) as usize; // which wavetable to use (rounded down)
    ...
    let frequency_difference = BASE_FREQUENCY * 2_f32.powi((wavetable_index as i32 - 1).max(0));
